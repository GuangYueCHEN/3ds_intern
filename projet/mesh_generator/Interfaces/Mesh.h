...
public:
  ...
  Mesh() = default;
	...
	size_t add_point(const Point3D & p);
	size_t add_triangle(const std::array<size_t,3> & indices);
	void remove_triangle(size_t iNumTriangle);
	void remove_vertex(size_t iNumVertex);
	
	std::vector<size_t> GetTrianglesAroundVertex(size_t vertix); //renvoie la liste des triangles qui contiennent un vertex
	std::vector<size_t> GetTrianglesAroundTriangles(const std::array<size_t,3> & indices);
	std::vector<size_t> GetTrianglesAroundTriangles(const Triangle & t);
	std::vector<size_t> GetTrianglesAroundEdge(size_t Vertex1, size_t Vertex2); //renvoie la liste des triangles qui partagent une edge donnée (sans ordre) par 2 vertices
